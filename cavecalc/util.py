    ...
    'bedrock_BaCa', 'soil_Sr', 'bedrock_SrCa', 'soil_O2', 'soil_R14C', 'atm_pCO2', 'atm_d13C'],
        ...
        if os.path.exists(input_ranges_csv): 
            ...
            for variable in input_ranges_data['Variable']: 
                if variable in self.s.settings: 
                    value = self.s.settings[variable]
                else: 
                    value = None  # Default to None if variable is not found
                    
                if value is not None:
                    # Update min and max 
                    existing_df.loc[existing_df['Variable'] == variable, 'Minimum'] = min( 
                        existing_df.loc[existing_df['Variable'] == variable, 'Minimum'].dropna().tolist() + [value] 
                        ) 
                    existing_df.loc[existing_df['Variable'] == variable, 'Maximum'] = max( 
                        existing_df.loc[existing_df['Variable'] == variable, 'Maximum'].dropna().tolist() + [value]
                        ) 
                else:  
                    # If the variable is not found in the existing DataFrame, add a new row 
                    if variable in self.s.settings:  
                        new_value = self.s.settings[variable]  # Access value from settings 
                    else: 
                        new_value = None  # Default to None if variable is not found 
