...
class Solution(object):
    ...
    @property
    def aragonite_string(self):
            """Returns an empty aragonite solid solution string for Iphreeqc."""
            
            if hasattr(self, '_aragonite_string'):
                return "\nUSE SOLID_SOLUTION 2\n"
            else:
                # create solid solution template
                self._aragonite_string = ARAGONITE_SS
                if self.s.settings['soil_Mg'] > 0 or \
                   self.s.settings['bedrock_MgCa'] > 0 or \
                   self.s.settings['bedrock_mineral'].capitalize() == 'Dolomite':
                    self._aragonite_string += SS_MG   # add Mg-bearing members
                if self.s.settings['soil_Sr'] > 0 or \
                   self.s.settings['bedrock_SrCa'] > 0:
                    self._aragonite_string += SS_SR   # add Sr-bearing members                
                if self.s.settings['soil_Ba'] > 0 or \
                   self.s.settings['bedrock_BaCa'] > 0:
    ...
    def open_diss_rxn_with_reprecip(self, Carbonate):
        ...
        if self.s.settings['bedrock_mineral'].capitalize() == 'Calcite':
            ...
            rxn_string += '\n' + self.aragonite_string  # Incorporating aragonite string
        elif self.s.settings['bedrock_mineral'].capitalize() == 'Dolomite':
            rxn_string += '\n' + self.calcite_string  # Consider dolomite
        else: 
            raise ValueError("Invalid precipitate_mineralogy specified.")
    ...
    def precipitate_rxn(self):
        ...
        Returns:
            (pq_str, desc_str) for adding to the iphreeqc buffer.
        """
        if self.s.settings['precipitate_mineralogy'].capitalize() == 'Calcite':
            return self.calcite_string, "CaCO3_precipitation"
        elif self.s.settings['precipitate_mineralogy'].capitalize() == 'Aragonite':
            return self.aragonite_string, "CaCO3_precipitation"
        else:
            raise ValueError("Invalid precipitate_mineralogy specified.")
...
class Simulator(object):
    ...
    def __init__(self, settings, id=0):     
        ...
        # set up reader to get thermodynamic data & reactions
