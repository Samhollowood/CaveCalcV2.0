...
class Evaluate(object):
    ...
    def plot_CDA(self,dir1,dir2): 
        ...
        df_main = pd.read_csv(matches_csv)
        ...
        for i, (key, mapped_col) in enumerate(column_mapping_main.items()):   
            ...
            if mapped_col in df_main.columns and key in df_test.columns: 
                ...
                for age in df_main[age_column_main].unique():   
                    age_mask = df_main[age_column_main] == age 
                    age_min = df_main[mapped_col][age_mask].min()
                    age_max = df_main[mapped_col][age_mask].max()


                # Calculate the x-axis range to apply relative offset 
                x_min, x_max = df_main[age_column_main].min(), df_main[age_column_main].max() 
                x_range = x_max - x_min 
                x_offset = x_range * relative_offset_fraction  # Apply offset proportionally 
                
                # Plot scatter points with offset applied to Measured Data 
                ax.scatter(df_main[age_column_main], df_main[mapped_col], marker='o', color='darkgreen', s=100, label=f'Modeled {key}')  
                
                # Apply relative offset to the x-coordinates of Measured Data 
                ax.scatter(df_test[age_column_test] + x_offset, df_test[key], marker='s', color='black', s=100, label=f'Measured {key}')  

                # Now plot the 'All outputs' sheet (black points) next to the df_main green points
                if mapped_col in df_all_outputs.columns: 
                    # Plot black scatter points for df_all_outputs (black) 
                    ax.scatter(df_all_outputs[age_column_main] - x_offset, df_all_outputs[mapped_col], marker='o', color='black', s=80, label=f'Output {key}', alpha=0.7)
        

                # Add vertical lines with ± tolerance for each point
                for j, age in enumerate(df_test[age_column_test]): 
                    ax.vlines(x=age + x_offset, ymin=df_test[key][j] - tolerance_value, ymax=df_test[key][j] + tolerance_value, color='black', linestyles='--')   
               
                ax.set_xlim(x_min - 4*x_offset, x_max + 4*x_offset)
...
'''    
    def generate_summary_statistics(self,dir1, dir2): 
        ...
        Parameters:
        - dir1: Path to the user's data file (Excel)
        - dir2: Path to the CaveCalc data file (Excel)
    
        Returns:
        - None
        """ 
        # Load the datasets
        df_user = pd.read_excel(dir1)
        ...
        columns_to_analyze = [col for col in columns_to_analyze if col in df_cavecalc.columns]

        # Calculate the median values for each variable across all ages
        cavecalc_medians = df_cavecalc.groupby('Age')[columns_to_analyze].median()
        ...
        with open(output_file, 'w') as f: 
            # Write header
            f.write(f"Summary Statistics Comparison\n")
            f.write(f"User Data File: {dir1}\n")
            f.write(f"CaveCalc Data File: {dir2}\n\n")
            
            # Write the number of rows (matches) in CaveCalc data
            num_matches = len(df_cavecalc)
            f.write(f"Number of matches: {num_matches}\n\n")
            
            # Write the median values for each variable 
            f.write("Median values for CaveCalc variables by age:\n")
            for age, row in cavecalc_medians.iterrows():  
                f.write(f"Age: {age}\n") 
                medians_line = " ".join(f"{col}: {row[col]:.2f}" for col in columns_to_analyze) 
                f.write(medians_line + "\n")
  
            # Count the number of rows per unique age in the CaveCalc data 
            if 'Age' in df_cavecalc.columns: 
                age_counts = df_cavecalc['Age'].value_counts() 
                f.write("Number of matches per Age:\n")
            for age, count in age_counts.items():
                f.write(f"  Age {age}: {count} matches\n") 
                f.write("\n") 
           ...
           # Dictionary to track ranges for each variable
            ...
            f.write("Model accepted range of variables for each Age:\n")
            for age in age_counts.index:
                age_data = df_cavecalc[df_cavecalc['Age'] == age]
                f.write(f"\nAge {age}:\n")
                
                for var in variables:
                    if var in age_data.columns:
                        min_val = age_data[var].min()
                        max_val = age_data[var].max()
                        ranges_by_age[var][age] = (min_val, max_val)
                        f.write(f"  {var} range: {min_val:.2f} to {max_val:.2f}\n")
                    else:
                        f.write(f"  {var}: Column not found\n")
                f.write("\n")

            # Determine which variables require changing
            f.write("Accepted model variables with a significant change:\n")
            significant_variables = set()
            for var, age_ranges in ranges_by_age.items():
                if len(age_ranges) > 1:
                    ...
                    for i in range(len(age_list)):
                        for j in range(i + 1, len(age_list)):
                            age1 = age_list[i]
                            age2 = age_list[j]
                            min1, max1 = age_ranges[age1]
                            min2, max2 = age_ranges[age2]
                    ...
                    if non_overlapping:
                        significant_variables.add(var)
                        f.write(f"\n{var}:\n")
                        for age, (min_val, max_val) in age_ranges.items():
                            f.write(f"  Age {age}: {min_val:.2f} to {max_val:.2f}\n") 
                            
            # Define the specific combinations and conclusions 
