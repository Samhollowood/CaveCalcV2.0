...
class CDAPlotter:
    ...
    def load_data(self, dir1, dir2):
        ...
        for key, path in csv_files.items():
            if key == 'matches':
                data[key] = pd.read_csv(path, on_bad_lines="skip",low_memory=False)
            else:
        ...
        self._test_cols = set(map(str, data['test'].columns.str.strip().str.lower()))
    ...
    def format_trace_metals(self, df_main):  
        bedrock_values, soil_values = {}, {}  
        col_map = {
        'd13C': ('bedrock_d13C', 'soil_d13C'),
        'd18O': ('bedrock_d18O', 'soil_d18O'),
        'd44Ca': ('bedrock_d44Ca', 'soil_d44Ca')}

        for metal in self.trace_metals:  
            bedrock_col, soil_col = col_map.get(metal, (f'bedrock_{metal}Ca', f'soil_{metal}Ca')) 
        ...
        def format_text(vals, is_bedrock):  
            return ','.join(
            f"{metal}: {','.join(map(str, v))} ‰" if metal in ['d44Ca', 'd18O', 'd13C'] 
            else f"{metal}/Ca: {','.join(map(str, v))}"
            for metal, v in vals.items() if len(v) > 0 and all(x != 0 for x in v) 
            )

        return format_text(bedrock_values, True), format_text(soil_values, False)
    ...
    def add_input_annotations(self, fig, input_ranges_df, bedrock_text, soil_text):
        ...
        # Which proxy keys are absent from the test CSV?
        missing_proxies = [p for p in self.column_mapping.keys() if p not in self._test_cols]

        # Nothing to list? Then don't render the grid.
        if not missing_proxies:
            return
        
        # Map to correct display casing
        def format_proxy_label(proxy):
            mapping = {
                'd13c': 'd13C',
                'd18o': 'd18O',
                'd44ca': 'd44Ca',
            ...
            return mapping.get(proxy.lower(), proxy)

        y_avail, x_spacing, y_spacing = 0.92, 0.05, 0.025
        for i, proxy in enumerate(missing_proxies):
    ...
    def plot_flow_path_controls(self, data):
        ...
        y_pos = 0.94 
        bedrock_values = [v for v in bedrock_text.split(',') if v.strip()] 
        
        for i, value in enumerate(bedrock_values): 
            x_pos = 0.17 + i * 0.05   # adjust 0.08 for tighter/looser spacing 
            fig.text(x_pos, y_pos, value, ha='center', va='center', fontsize=10)
        ...
        if not row.empty:
            ...
            fig.text(0.20, y_pos - (len(bedrock_values) + 1) * 0.020, gas_text, 
                    ha='center', va='center', fontsize=10)
    ...
    def plot_CDA(self, dir1, dir2):
        ...
        figures = [] 
        for plot_func in [self.plot_co2_processes, self.plot_flow_path_controls, self.plot_model_data_comparison]: 
            fig = plot_func(data) 
            figures.append(fig) 
            plt.show()  # shows one figure at a time, blocking until closed 
        return figures
