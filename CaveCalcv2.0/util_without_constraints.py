...
class PostProcessor(object):
    ...
    def EventAnalyser(self): 
        ...
        file_path = self.s.settings.get('user_filepath') 
        ...
        for index, d13C_value in enumerate(d13C_data): 
            ...
            residual_check = abs(residual) <= tolerance
            ...
            if residual_check and MgCa_check and dcp_check and d44Ca_check and SrCa_check and BaCa_check and UCa_check: 
                ...
                results_with_constraints.append(extended_record)
                match_found = True 
                
            # Results without residual constraints
            extended_record_no_constraints = base_record.copy() 
            if MgCa_data: 
                extended_record_no_constraints.update({'MgCa': MgCa_data[index], 'CaveCalc MgCa': MgCa_spel, 'MgCa Residual': MgCa_residual}) 
            if dcp_data: 
                extended_record_no_constraints.update({'DCP': dcp_data[index], 'CaveCalc DCP': dcp_spel, 'DCP Residual': dcp_residual}) 
            if d44Ca_data: 
                extended_record_no_constraints.update({'d44Ca': d44Ca_data[index], 'CaveCalc d44Ca': d44Ca_spel, 'd44Ca Residual': d44Ca_residual}) 
            if SrCa_data: 
                extended_record_no_constraints.update({'SrCa': SrCa_data[index], 'CaveCalc SrCa': SrCa_spel, 'SrCa Residual': SrCa_residual}) 
            if BaCa_data: 
                extended_record_no_constraints.update({'BaCa': BaCa_data[index], 'CaveCalc BaCa': BaCa_spel, 'BaCa Residual': BaCa_residual}) 
            if UCa_data: 
                extended_record_no_constraints.update({'UCa': UCa_data[index], 'CaveCalc UCa': UCa_spel, 'UCa Residual': UCa_residual})

            results_without_constraints.append(extended_record_no_constraints) 
            match_found = True 
            
            # Proceed with results processing only if a match was found 
            if match_found: 
                new_results_with_constraints_df = pd.DataFrame(results_with_constraints)
                new_results_without_constraints_df = pd.DataFrame(results_without_constraints) 
                
                # Combine new results with existing results 
                if not hasattr(self, 'results_df_with_constraints'): 
                    self.results_df_with_constraints = pd.DataFrame()

                if not hasattr(self, 'results_df_without_constraints'): 
                    self.results_df_without_constraints = pd.DataFrame() 
                    
                if not self.results_df_with_constraints.empty: 
                    combined_df_with_constraints = pd.concat([self.results_df_with_constraints, new_results_with_constraints_df], ignore_index=True)
                else: 
                    combined_df_with_constraints = new_results_with_constraints_df
        
                if not self.results_df_without_constraints.empty: 
                    combined_df_without_constraints = pd.concat([self.results_df_without_constraints, new_results_without_constraints_df], ignore_index=True) 
                else: 
                    combined_df_without_constraints = new_results_without_constraints_df

                # Update the aggregated DataFrames
                self.results_df_with_constraints = combined_df_with_constraints
                self.results_df_without_constraints = combined_df_without_constraints 
                
                # Save the cumulative DataFrames to EventAnalyser.xlsx 
                if not os.path.exists(excel_file): 
                    # If the file does not exist, create it and write the DataFrames with headers 
                    with pd.ExcelWriter(excel_file, engine='openpyxl') as writer: 
                        self.results_df_without_constraints.to_excel(writer, sheet_name='Results_Without_Constraints', index=False) 
                        self.results_df_with_constraints.to_excel(writer, sheet_name='Results_With_Constraints', index=False) 
                        print(f"Created new file '{excel_file}' and saved results.") 
                else: 
                    # If the file exists, append new results to it 
                    with pd.ExcelWriter(excel_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer: 
                        # Write with headers if the sheet is new or needs headers 
                        results_written = False 
                        
                        if 'Results_With_Constraints' in writer.sheets: 
                            startrow = writer.sheets['Results_With_Constraints'].max_row 
                            if startrow > 1:  # Ensure there is data to append  
                                results_written = True 
                                headers = new_results_without_constraints_df.columns.tolist()
                                self.results_df_with_constraints.to_excel(writer, sheet_name='Results_With_Constraints', index=False, header=False, startrow=startrow) 
                        else: 
                            results_written = True 
                            headers = new_results_without_constraints_df.columns.tolist()
                            self.results_df_with_constraints.to_excel(writer, sheet_name='Results_With_Constraints', index=False, header=headers)
                            
                                
                        if 'Results_Without_Constraints' in writer.sheets: 
                            # Copy headings from 'Results_Without_Constraints' to 'Results_With_Constraints' 
                            if 'Results_With_Constraints' not in writer.sheets: 
                                headers = new_results_without_constraints_df.columns.tolist() 
                                self.results_df_with_constraints.to_excel(writer, sheet_name='Results_With_Constraints', index=False, header=headers) 
                            else: 
                                self.results_df_without_constraints.to_excel(writer, sheet_name='Results_Without_Constraints', index=False, header=False, startrow=writer.sheets['Results_Without_Constraints'].max_row) 
                                
                        if results_written: 
                            print('Match with measured data!')
