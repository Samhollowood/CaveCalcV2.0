...
class PostProcessor(object):
    ...
    def EventAnalyser(self): 
        ...
        for index, d13C_value in enumerate(d13C_data): 
            ...
            if MgCa_data: 
                extended_record_no_constraints.update({'MgCa': MgCa_data[index], 'CaveCalc MgCa': MgCa_spel})
            if dcp_data:
                extended_record_no_constraints.update({'DCP': dcp_data[index], 'CaveCalc DCP': dcp_spel})
            if d44Ca_data:
                extended_record_no_constraints.update({'d44Ca': d44Ca_data[index], 'CaveCalc d44Ca': d44Ca_spel})
            if SrCa_data:
                extended_record_no_constraints.update({'SrCa': SrCa_data[index], 'CaveCalc SrCa': SrCa_spel})
            if BaCa_data:
                extended_record_no_constraints.update({'BaCa': BaCa_data[index], 'CaveCalc BaCa': BaCa_spel})
            if UCa_data:
                extended_record_no_constraints.update({'UCa': UCa_data[index], 'CaveCalc UCa': UCa_spel})
            ...
            if match_found: 
                ...
                if not os.path.exists(excel_file): 
                    # If the file does not exist, create it and write the DataFrames 
                    ...
                    with pd.ExcelWriter(excel_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer: 
                        startrow = writer.sheets['Results_With_Constraints'].max_row if 'Results_With_Constraints' in writer.sheets else 0
                        self.results_df_with_constraints.to_excel(writer, sheet_name='Results_With_Constraints', index=False, header=startrow==0, startrow=startrow)
                
                        startrow = writer.sheets['Results_Without_Constraints'].max_row if 'Results_Without_Constraints' in writer.sheets else 0
                        self.results_df_without_constraints.to_excel(writer, sheet_name='Results_Without_Constraints', index=False, header=startrow==0, startrow=startrow) 
                        
                    print(f"Match! Appended results to existing file '{excel_file}'.")
