...
class PostProcessor(object):
    ...
    def EventAnalyser(self):
        ...
        if self.s.settings['precipitate_mineralogy'] == 'Calcite':
            ...
            dcp_spel = self.s.output.get('DCP', [])
            dcp_spel = dcp_spel[-1] if dcp_spel else None
            d44Ca_spel = self.s.output.get('d44Ca_Calcite', [])
            d44Ca_spel = d44Ca_spel[-1] if d44Ca_spel else None
        elif self.s.settings['precipitate_mineralogy'] == 'Aragonite':
            ...
            dcp_spel = self.s.output.get('DCP', [])
            dcp_spel = dcp_spel[-1] if dcp_spel else None
            d44Ca_spel = self.s.output.get('d44Ca_Aragonite', [])
            d44Ca_spel = d44Ca_spel[-1] if d44Ca_spel else None
        else:
        ...
        for index, d13C_value in enumerate(d13C_data): 
            adjusted_index = index + 1  # Increment index by 1 to start from 1
            ...
            if MgCa_data is None and dcp_data is None and d44Ca_data is None: 
                # Check if residual is within tolerance and append the base record 
                if abs(residual) <= tolerance: 
                    results.append(base_record)
            elif MgCa_data is not None and dcp_data is None and d44Ca_data is None:
                # If MgCa_data is not None, iterate over MgCa_data 
                for Mg_index, MgCa_value in enumerate(MgCa_data): 
                    ...
                    if abs(residual) <= tolerance and abs(MgCa_residual) <= mg_tolerance: 
                        # Create a new record with MgCa specific data after 'd13C residual' 
                        ...
                        match_found = True 
            elif MgCa_data is not None and dcp_data is not None and d44Ca_data is None:  
                MgCa_residual = MgCa_spel - MgCa_data[index]
                dcp_residual = dcp_spel - dcp_data[index]  
                if abs(residual) <= tolerance and abs(MgCa_residual) <= mg_tolerance and abs(dcp_residual) <= dcp_tolerance:  
                    # Create a new record with MgCa specific data after 'd13C residual'  
                    extended_record = base_record.copy()  
                    extended_record.update({  
                        'MgCa': dcp_data[index], 
                        ...
                        'DCP residual': dcp_residual 
                    })  
                    results.append(extended_record)
                    match_found = True
            elif MgCa_data is not None and dcp_data is not None and d44Ca_data is not None:  
                  MgCa_residual = MgCa_spel - MgCa_data[index]
                  dcp_residual = dcp_spel - dcp_data[index]  
                  d44Ca_residual = d44Ca_spel - d44Ca_data[index]
                  if abs(residual) <= tolerance and abs(MgCa_residual) <= mg_tolerance and abs(dcp_residual) <= dcp_tolerance and abs(d44Ca_residual) <= d44Ca_tolerance:  
                      # Create a new record with MgCa specific data after 'd13C residual'  
                      extended_record = base_record.copy()  
                      extended_record.update({  
                          'MgCa': dcp_data[index], 
                          ...
                          'd44Ca residual': d44Ca_residual,
                          
                      })  
                      results.append(extended_record)
                      match_found = True   
            elif MgCa_data is not None and dcp_data is None and d44Ca_data is not None:  
                    MgCa_residual = MgCa_spel - MgCa_data[index]
                    d44Ca_residual = d44Ca_spel - d44Ca_data[index]
                    if abs(residual) <= tolerance and abs(MgCa_residual) <= mg_tolerance and abs(d44Ca_residual) <= d44Ca_tolerance:  
                        # Create a new record with MgCa specific data after 'd13C residual'  
                        extended_record = base_record.copy()  
                        extended_record.update({  
                            'MgCa': dcp_data[index], 
                        ...
                        results.append(extended_record)
                        match_found = True 
            elif MgCa_data is None and dcp_data is not None and d44Ca_data is not None:  
                      dcp_residual = dcp_spel - dcp_data[index]
                      d44Ca_residual = d44Ca_spel - d44Ca_data[index]
                      if abs(residual) <= tolerance and abs(MgCa_residual) <= mg_tolerance and abs(d44Ca_residual) <= d44Ca_tolerance:  
                          # Create a new record with MgCa specific data after 'd13C residual'  
                          ...
                          results.append(extended_record)
                          match_found = True
            elif MgCa_data is None and dcp_data is not None and d44Ca_data is None:  
                        dcp_residual = dcp_spel - dcp_data[index]
                        d44Ca_residual = d44Ca_spel - d44Ca_data[index]
                        if abs(residual) <= tolerance and abs(MgCa_residual) <= mg_tolerance and abs(d44Ca_residual) <= d44Ca_tolerance:  
                            # Create a new record with MgCa specific data after 'd13C residual'  
                            ...
                            results.append(extended_record)
                            match_found = True  
            elif MgCa_data is None and dcp_data is None and d44Ca_data is not None:  
                         dcp_residual = dcp_spel - dcp_data[index]
                         d44Ca_residual = d44Ca_spel - d44Ca_data[index]
                         if abs(residual) <= tolerance and abs(MgCa_residual) <= mg_tolerance and abs(d44Ca_residual) <= d44Ca_tolerance:  
                             # Create a new record with MgCa specific data after 'd13C residual'  
