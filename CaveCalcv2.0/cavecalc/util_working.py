...
class PostProcessor(object):
    ...
    def EventAnalyser(self):
        ...
        for index, d13C_value in enumerate(d13C_data):
            adjusted_index = index + 1  # Increment index by 1 to start from 1 instead of 
            ...
            if MgCa_data is None:
                ...
                for Mg_index, MgCa_value in enumerate(MgCa_data): 
                    MgCa_residual = MgCa_spel - MgCa_value 
                    ...
                    'CaveCalc d13C': d13C_spel,
                    'd13C residual': residual,
                    'MgCa': MgCa_data[Mg_index],
                    'CaveCalc MgCa': MgCa_spel,
                    'MgCa Residual': MgCa_residual,
                    'soil_pCO2 (ppmv)': soil_pCO2,
                    'Soil d13C': d13Csoil,
                    'cave_pCO2 (ppmv)': cave_pCO2,
                    'f_{ca}': f_ca,
        ...
        # Now perform aggregation based on the full history of 'Results'
        try:
            # Load all historical data from 'Results'
            full_results_df = pd.read_excel(excel_file, sheet_name='Results')

            # Group by 'd13C_index' and calculate the mean
            aggregate_df = full_results_df.groupby('d13C_index').median().reset_index()
            
            # List of columns for which to calculate quartiles
            quartile_columns = ['soil_pCO2 (ppmv)', 'Soil d13C', 'cave_pCO2 (ppmv)', 'f_{ca}', 'gas volume (L/kg)', 'T(°C)']

            # Calculate the 25th and 75th percentiles for the specified variables
            percentile_25_df = full_results_df.groupby('d13C_index')[quartile_columns].quantile(0.25).reset_index()
            percentile_75_df = full_results_df.groupby('d13C_index')[quartile_columns].quantile(0.75).reset_index()

            # Replace NaN values with 0 in both percentile DataFrames
            percentile_25_df = percentile_25_df.fillna(0)
            percentile_75_df = percentile_75_df.fillna(0)

            # Rename columns in percentile DataFrames to match the format 'variable_name Lower Quartile' and 'variable_name Upper Quartile'
            percentile_25_df.columns = [f'{col} Lower Quartile' if col in quartile_columns else col for col in percentile_25_df.columns]
            percentile_75_df.columns = [f'{col} Upper Quartile' if col in quartile_columns else col for col in percentile_75_df.columns]

             # Insert the lower and upper quartile columns next to the corresponding variable columns
            for col in reversed(quartile_columns):  # Reverse order to correctly insert next to each variable  
                lower_col = f'{col} Lower Quartile' 
                upper_col = f'{col} Upper Quartile' 
            
                # Find the position of the original column in aggregate_df 
                col_position = aggregate_df.columns.get_loc(col) + 1 
                # Insert the upper quartile column immediately after the original column
                aggregate_df.insert(col_position, upper_col, percentile_75_df[upper_col])
        
                # Insert the lower quartile column immediately after the original column
                aggregate_df.insert(col_position, lower_col, percentile_25_df[lower_col])


            # Save the new aggregated results to the 'Aggregate' sheet, replacing the old one
            with pd.ExcelWriter(excel_file, engine='openpyxl', mode='a', if_sheet_exists='replace') as writer:
                aggregate_df.to_excel(writer, sheet_name='Aggregate', index=False)

            print("Aggregate sheet replaced with new data")
            print('Match!')
