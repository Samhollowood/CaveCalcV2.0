...
class PostProcessor(object):
    ...
    def EventAnalyser(self): 
        ...
        results = []
        residuals = []  # List to store residuals with no constraints
        ...
        for index, d13C_value in enumerate(d13C_data): 
            adjusted_index = index + 1  # Increment index by 1 to start from 1 instead of 0
            residual = d13C_spel - d13C_value
            MgCa_residual = None

        if MgCa_data is not None:
            # Calculate MgCa residual if MgCa_data is present
            MgCa_value = MgCa_data[index] if index < len(MgCa_data) else None
            if MgCa_value is not None:
                MgCa_residual = MgCa_spel - MgCa_value

        # Append residual data with MgCa residual if available
        residuals.append({
        ...
        if match_found or results: 
            # Process results and save directly
        ...
        # Save both the 'Results' and 'Residuals' sheets to EventAnalyser.xlsx
        if not os.path.exists(excel_file):
            ...
            print(f"Created new file '{excel_file}' and saved results and residuals.")
        else:
            # If the file exists, append new results and residuals to it
            book = load_workbook(excel_file)
            with pd.ExcelWriter(excel_file, engine='openpyxl', mode='a', if_sheet_exists='overlay') as writer:
                # Append 'Results' sheet
                if 'Results' in book.sheetnames:
                    startrow = book['Results'].max_row
                    self.results_df.to_excel(writer, sheet_name='Results', index=False, header=False, startrow=startrow)
                else:
                    self.results_df.to_excel(writer, sheet_name='Results', index=False)

                # Append 'Residuals' sheet
                if 'Residuals' in book.sheetnames:
                    startrow = book['Residuals'].max_row
                    residuals_df.to_excel(writer, sheet_name='Residuals', index=False, header=False, startrow=startrow)
                else:
                    residuals_df.to_excel(writer, sheet_name='Residuals', index=False)

            print(f"Appended results and residuals to existing file '{excel_file}'.")
