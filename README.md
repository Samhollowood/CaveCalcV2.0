...
The git command (https://git-scm.com/downloads) is needed for this particular method of installation. Alternatively, users may download the repository as a zip-file, locally.
...
`run_models.py` uses default model inputs, while `run_CDA.py` has a select number of inputs, over a range, intended to guide the user on the CDA process. These model inputs can be **added, removed, or modified** to suit the users study. A complete list of inputs is available in **manual.pdf**, Table 2.  
...
A template (`Example.input.csv`) is provided for user data. Modify it by adding/removing proxies and data as needed. Alternatively, the inputs for the CDA may be found in the CDA Settings field within the GUI, which allows users to import their measured data and define tolerance intervals.
